...
def load_and_preprocess_data(file_path):
    # CSV 로드 (컬럼명: Comments, Ratings)
    df = pd.read_csv(file_path)
    
    processed_data = []
    for _, row in df.iterrows():
        try:
            score = int(row['Ratings'])
            review = str(row['Comments'])
            
            # 10점 만점 기준 3진 분류 매핑 (중립 구간 확보가 핵심!)
            if score <= 4:
                label = 0  # 부정 (1~4점)
            elif 5 <= score <= 7:
                label = 1  # 중립 (5~7점: "보통", "볼만함")
            else:
                label = 2  # 긍정 (8~10점: "강추", "최고")
                
            processed_data.append([review, label])
        except (ValueError, TypeError):
            continue

    new_df = pd.DataFrame(processed_data, columns=['text', 'label'])
    
    # 클래스 불균형 확인용 출력
    print("📊 [Kaggle] 라벨별 데이터 분포:")
...
# --- 실행 ---
file_path = 'kalki_movie_reviews.csv'
